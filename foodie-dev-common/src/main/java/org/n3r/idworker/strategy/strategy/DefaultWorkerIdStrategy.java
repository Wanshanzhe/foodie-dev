package org.n3r.idworker.strategy.strategy;
...
public class DefaultWorkerIdStrategy implements WorkerIdStrategy {
    ...
    long workerId;
    org.n3r.idworker.strategy.FileLock fileLock;
    ...
    private long findAvailWorkerId() {
        ...
        for (File lockFile : idWorkerHome.listFiles()) {
            ...
            org.n3r.idworker.strategy.FileLock fileLock = new FileLock(lockFile);
            if (!fileLock.tryLock()) {
                fileLock.destroy();
                continue;
            }
