...
public class SSOController {
    ...
    /**
     * CAS的统一登录接口
     *  目的：1.登录创建用户的全局会话 -> uniqueToken
     *       2.创建用户全局门票，用户表示在cas端是否登录 -> userTicket
    ...
    public String doLogin(String username,
        ...
        redisOperator.set(REDIS_USER_TOKEN + ":" + usersResult.getId(), JsonUtils.objectToJson(usersVO));

        //3.生成ticket门票，全局门票，代表用户在CAS端登录过
        String userTicket = UUID.randomUUID().toString().trim();

        //3.1用户全局门票需要放在CASd端的cookie中
        setCookie(COOKIE_USER_TICKET,userTicket,response);

        //4.userTicket关联用户id，并且放入到redis中，代表这个用户有门票了，可以在各个景区游玩
        redisOperator.set(REDIS_USER_TICKET + ":" + userTicket,usersResult.getId());
    ...
    public IMOOCJSONResult verifyTmpTicket(String tmpTicket,
        ...
        if (StringUtils.isNotBlank(tmpTicketValue)){
        ...
        //1.验证并获取用户的userTicket
        String userTicket = getCookie(request,COOKIE_USER_TICKET);
        String userId = redisOperator.get(REDIS_TMP_TICKET + ":" + userTicket);
        if (StringUtils.isBlank(userId)){
            return IMOOCJSONResult.errorUserTicket("用户票据异常");
        }
        //2.验证门票对应的user会话是否存在
